    ...
    <form onSubmit={submit} className="mx-auto max-w-3xl rounded-[2rem] border border-white/15 bg-white p-4 text-slate-950 shadow-2xl shadow-cyan-950/30 sm:p-5">
      <div className="flex items-start justify-between gap-4 px-1 py-2">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Pilih paket, lalu lanjut bayar QRIS.</p>
        </div>
      ...
      <details className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        <summary className="cursor-pointer text-sm font-black text-slate-700">Nama/HP opsional</summary>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <input value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="Nomor HP" className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100" />
        </div>
      </details>
      ...
      {selected && (
        <div className="mt-4 flex items-center justify-between rounded-3xl bg-gradient-to-br from-slate-950 to-slate-800 p-4 text-white">
          ...
          <div className="text-right text-xs font-bold leading-5 text-cyan-100">Bayar pakai<br />QRIS</div>
        </div>
      )}
